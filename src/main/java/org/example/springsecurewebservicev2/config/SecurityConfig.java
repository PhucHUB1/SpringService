...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.authorizeHttpRequests(authorizeRequests -> authorizeRequests.requestMatchers("/api/books", "/api/books/*", "/api/publishers", "/api/publishers/*", "/api/publications", "/api/publications/*").permitAll().requestMatchers("/api/employees", "/api/employees/*").hasRole("EMPLOYEE").requestMatchers("/api/admin").hasRole("ADMIN").anyRequest().authenticated()).formLogin(loginForm -> loginForm.permitAll().defaultSuccessUrl("/api/home")).logout(logout -> logout.deleteCookies("JSESSIONID")).httpBasic(Customizer.withDefaults());
        http.csrf().disable();
